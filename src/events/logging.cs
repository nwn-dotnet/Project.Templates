namespace NWN.Events
{
    public class EventLogger
    {
        ...
        public static void HookAllMessages()
        {
            ...
            ClientConnectEvent.BeforeClientConnect += (e) => Log($"{e.PlayerName} connecting with {e.PlayerCDKey} from {e.PlayerIP}");
            ClientConnectEvent.AfterClientConnect += (e) => Log($"{e.PlayerName} connected with {e.PlayerCDKey} from {e.PlayerIP}");
            ...
            CombatRoundEvent.BeforeCombatRoundStart += (e) => Log($"{e.Creature.Name} entering new round: {e.Target.Name}");
            CombatRoundEvent.AfterCombatRoundStart += (e) => Log($"{e.Creature.Name} entered new round: {e.Target.Name}");
            ...
            InventorySelectPanelEvent.BeforeSelectPanel += (e) => Log($"{e.Player.Name} switching to inventory screen {e.SelectedPanelIndex}");
            InventorySelectPanelEvent.AfterSelectPanel += (e) => Log($"{e.Player.Name} switched to inventory screen {e.SelectedPanelIndex}");
            ...
            WalkInputEvent.BeforeInputWaypoint += (e) => Log($"{e.Player.Name} wants to move to {e.Area.Name} ({e.Location.x}, {e.Location.y}) {e.Running}");
            WalkInputEvent.AfterInputWaypoint += (e) => Log($"{e.Player.Name} moving to {e.Area.Name} ({e.Location.x}, {e.Location.y}) {e.Running}");
        }
    }
}