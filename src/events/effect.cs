namespace NWN.Events {
	public class EffectEvent : NWNXEvent {
		...
		public EffectEvent(string script) {
			EventType = script;
		}
		...
		public int GetIntParam(int ndx) {
			return GetEventInt($"INT_PARAM_{ndx}");
		}

		public float GetFloatParam(int ndx) {
			return GetEventFloat($"FLOAT_PARAM_{ndx}");
		}

		public string GetStringParam(int ndx) {
			return GetEventString($"STRING_PARAM_{ndx}");
		}

		public NWObject GetObjectParam(int ndx) {
			return GetEventObject($"OBJECT_PARAM_{ndx}").AsObject();
		}
		...
		public static void EventHandler(string script) {
			...
			switch (script) {
				case BEFORE_EFFECT_APPLIED:
					BeforeEffectApplied(e);
					break;
				case AFTER_EFFECT_APPLIED:
					AfterEffectApplied(e);
					break;
				case BEFORE_EFFECT_REMOVED:
					BeforeEffectRemoved(e);
					break;
				case AFTER_EFFECT_REMOVED:
					AfterEffectRemoved(e);
					break;
			}
		}
	}
