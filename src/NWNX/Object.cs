  ...
  public class Object {
    ...
    public static LocalVariable GetLocalVariable(uint obj, int index) {
      ...
      var lv = new LocalVariable {
        Key = Internal.NativeFunctions.nwnxPopString(), Type = (LocalVarType) Internal.NativeFunctions.nwnxPopInt()
      };
      return lv;
    }
    ...
    // Gets the given object's internal NWNX_OBJECT_TYPE_INTERNAL_*
    public static InternalObjectType GetInternalObjectType(uint obj) {
    ...
    // nwObject The object receiving oItem, must be a Creature, Placeable, Store or Item
    // nwItem The item.
    // TRUE on success.
    public static bool AcquireItem(uint nwObject, uint nwItem) {
      Internal.NativeFunctions.nwnxSetFunction(PLUGIN_NAME, "AcquireItem");
      Internal.NativeFunctions.nwnxPushObject(nwItem);
